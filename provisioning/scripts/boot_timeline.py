...
`task up` gets Lima VMs running fairly quickly, but the ArgoCD-driven GitOps
tree (Rook-Ceph, Prometheus, Loki, ...) takes much longer to go fully
Healthy, and until now that number was a stopwatch guess (the Taskfile's
own "~10-20 min first run" comment was never actually measured). This polls
`kubectl get application -n argocd` on a fixed interval, logs every (sync,
health) transition it observes with elapsed time since this script started,
and writes the full timeline to a CSV for later before/after comparison
across boot-speed changes.

Usage: run in its own terminal, ideally started right when `task up` is
kicked off so the timeline covers the full VM-boot -> API-reachable ->
every-app-Healthy window, not just the ArgoCD-visible portion:

    task up &
    python3 provisioning/scripts/boot_timeline.py

No webhook is configured against this cluster's ArgoCD, so polling is the
only external option; Application *status* (sync/health) still updates
promptly from the controller's own in-cluster watch, independent of the
git-polling resync interval, so this doesn't inherit that delay.

If an Application is known to sit permanently Progressing/Degraded by
design (the way thump-test's hostNetwork-mode Cilium Gateway never reports
Programmed=True), pass its name via --exclude one or more times so the
"all healthy" completion condition doesn't wait on it forever.
"""
...
def main() -> None:
    ...
    parser.add_argument("--context", default=DEFAULT_CONTEXT, help=f"kubectl context (default: {DEFAULT_CONTEXT})")
    ...
    parser.add_argument(
        ...
        default=[],
        metavar="APP_NAME",
        help="Application name known to never report Healthy by design; repeatable",
    )
    args = parser.parse_args()
    known_permanently_progressing = set(args.exclude)
